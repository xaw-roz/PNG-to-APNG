...
def apng_main(src_path, filename, delayTime):
    all_files = glob.glob(os.path.join(src_path, '*.png'))
    all_files.sort(key=tokenize)
    APNG.from_files([*all_files], delay=int(delayTime)).save(os.path.join(filename+'.png')) 


if __name__ == "__main__":

    apng_main(sys.argv[1], sys.argv[2], sys.argv[3])
